...
# To initailize a sample=samples needed to be normalized. For sample which donot need normalization, normalize=False
def initialize_sample(samples, normalize=True):
    num_amplitudes = len(samples)
    assert ispow2(num_amplitudes), 'len(amplitudes) should be power of 2'
    
    def ispow2(num):  # A function to check if 'num' is power of 2
    ...
    qc.initialize(ampls, [i for i in range(num_qubits)])
...
def getBlochSequence(path,qc_sequence):
    '''plot bloch sphere sequence and save it
       to a folder for gif movie creation'''
    try:
        os.mkdir(path)
    except:
        print('Directory already exist')
    for i,qc in enumerate(qc_sequence):
        getBlochSphere(qc).savefig(path+"rot_"+str(i)+".png")
    return

        
def getBlochGif(qc_sequence,path,fname,fps,remove = True):
    '''create gif movie from provided images'''
    file_list = glob.glob(path + "/*.png") 
    list.sort(file_list, key=lambda x: int(x.split('rot_')[1].split('.png')[0]))
